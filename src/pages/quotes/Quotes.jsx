...
import { Search, ChevronDown, ChevronUp, Download, Trash2, Edit2 } from 'lucide-react';
    ...
    <div className="container mx-auto p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex items-center space-x-4">
          ...
          <Link to={'/quotes/addquotes'} className="bg-indigo-500 text-white px-4 py-2 rounded-md hover:bg-indigo-600 shadow-md">
            New Quote
          </Link>
        </div>
      </div>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {quotes.map((quote) => (
                <tr key={quote.id} className="hover:bg-gray-100">
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900" onClick={() => navigate(`/quotes/${quote.id}`)}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{new Date(quote.quoteDate).toLocaleDateString()}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{new Date(quote.quoteDueDate).toLocaleDateString()}</td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <button onClick={() => deleteQuote(quote.id)} className="text-red-600 hover:text-red-800 mr-3"><Trash2 className="h-5 w-5" /></button>
                    <button onClick={() => navigate(`/quotes/updatequote/${quote.id}`)} className="text-blue-600 hover:text-blue-800"><Edit2 className="h-5 w-5" /></button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {hasMore && !loading && (
        <div className="flex justify-center mt-6">
          <button onClick={() => setPage(prev => prev + 1)} className="px-6 py-2 bg-gray-300 hover:bg-gray-400 rounded-md shadow-md">Load More</button>
        </div>
      )}
    </div>
  );
