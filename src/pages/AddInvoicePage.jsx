...
export default function InvoiceComponent() {
  ...
  function generatePDF() {
    ...
    // Add logo
    doc.addImage(logoImage, "JPEG", 10, 10, 40, 20); // Adjust the position and size as needed
    ...
    doc.setFontSize(12);
    doc.text(`Client Address: ${invoice.shippingAddressId}`, 10, 40); // Display selected address
    doc.text(`Invoice Number: ${invoice.invoiceNumber}`, 150, 30);
    doc.text(`Date: ${new Date(invoice.invoiceDate).toLocaleDateString()}`, 150, 37);
    doc.text(`Due Date: ${new Date(invoice.invoiceDueDate).toLocaleDateString()}`, 150, 44);
    ...
    if (client) {
        doc.text(client.name || `${client.firstName} ${client.lastName}`, 10, 67);
    }
    ...
    doc.setFontSize(12);
    doc.text(`Subtotal: Rs.${invoice.subTotal.toFixed(2)}`, 150, finalY);
    finalY += 7;
    doc.text(`Tax: Rs.${invoice.totalTax.toFixed(2)}`, 150, finalY);
    finalY += 7;
    doc.text(`Discount: Rs.${invoice.discount.toFixed(2)}`, 150, finalY);
    finalY += 7;
    doc.setFontSize(14);
    doc.text(`Total: Rs.${invoice.total.toFixed(2)}`, 150, finalY);

    // Footer Notes
    doc.setFontSize(10);
    doc.text("Notes:", 10, finalY + 20);
    doc.text(invoice.notes || "Thank you for your business!", 10, finalY + 27);

    // Stamp and Sign Placeholder
    doc.setFontSize(12);
    doc.text("Stamp:", 10, 260); // Adjust Y-position as needed
    doc.text("Sign:", 150, 260); // Adjust Y-position as needed
              ...
              <div>
                ...
                <div className="pt-2 flex items-center gap-5">
                  ...
                  <button
                    type="submit"
                    onClick={generatePDF}
                    className="bg-green-600 text-white w-32 h-11 rounded hover:bg-blue-700 transition-colors duration-200"
                  >
                    Download Inv
                  </button>
                  <button
